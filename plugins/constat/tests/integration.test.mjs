...
test('espace complet : démonstration → validation ACH → proposition, sans extension',async()=>{
 ...
 let stockage={},revision=0,propositions=[];
 ...
 assert.equal(propositions.length,1,$('etat').textContent);assert.equal(propositions[0].champs.probabilite,.8);
 assert.match($('etat').textContent,/file de validation/);assert.ok(Object.values(stockage).some(v=>Array.isArray(v)&&v.some(e=>e.t==='prisme-proposition')));
});
